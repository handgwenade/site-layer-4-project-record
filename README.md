...
The participation guide uses its own 1200 × 630 sharing image at `public/social/participation-guide-share-v1.png`, selected by the page's `image` override in `social-sharing.json`. Other pages keep the default artwork. `scripts/render-participation-share.mjs` reproduces this graphic with Sharp (`SL4_SHARP_PATH` may specify the installed module path); it reads dates and times from the participation page's county/federal cards. After a date change, regenerate and review the image, update its alt text and manifest entry, and use a new versioned filename so social crawlers can fetch the replacement. Previously shared previews may still retain the old artwork.
